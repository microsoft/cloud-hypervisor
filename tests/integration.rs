...
fn fw_path(_fw_type: FwType, vcpus: String, console: String) -> String {
    ...
    #[cfg(target_arch = "x86_64")]
    fw_path.push(format!("linux{}-{}.bin", vcpus, console));
    // {
    //     match _fw_type {
...
mod common_parallel {
    ...
    fn test_api_create_boot() {
        ...
        thread::sleep(std::time::Duration::new(600, 0));
    ...
    fn test_pci_bar_reprogramming() {
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            println!("========> Test: Guest after PCI rescan ip a: {}", &guest.ssh_command("ip a").unwrap());
    ...
    fn test_disk_hotplug() {
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            println!("========> Test: Guest lsblk before: {}", &guest.ssh_command("lsblk").unwrap());
            println!("========> Test: Guest lspci before: {}", &guest.ssh_command("lspci").unwrap());

            //Rescan the PCI devices before checking if vdc exist, change permission to write, default is 200
            println!("========> Test: Guest PCI Rescan chmod: {}", &guest.ssh_command("sudo chmod 222 /sys/bus/pci/rescan").unwrap());
            println!("========> Test: Guest PCI Rescan: {}", &guest.ssh_command("sudo echo 1 > /sys/bus/pci/rescan").unwrap());

            // Check that /dev/vdc exists and the block size is 16M.
            println!("========> Test: Guest lsblk after: {}", &guest.ssh_command("lsblk").unwrap());
            println!("========> Test: Guest lspci after: {}", &guest.ssh_command("lspci").unwrap());
            assert_eq!(
                guest
                    .ssh_command("lsblk | grep vdc | grep -c 16M")
                    .unwrap()
    ...
    fn _test_net_hotplug(pci_segment: Option<u16>) {
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            println!("========> Test: Guest total interface: {}", &guest.ssh_command("ip -o link").unwrap());
